...
const selected = new Set(Array.isArray(saved?.ids) ? saved.ids.filter(id => typeof id==='string' && /^[A-Za-z0-9_.-]{1,120}$/.test(id)).slice(0, 3) : []);
...
for (const capability of [...new Set(catalog.flatMap(g => g.capabilities))].sort()) { const option=node('option','',pretty(capability));option.value=capability;$('capability').append(option); }
...
function languageLabel(game){const values=game.interfaceLanguages||['unknown'];return values.includes('en')?'English':values.includes('zh-CN')?'Simplified Chinese':values[0]==='unknown'?'Language unknown':values.join(', ');}
...
let currentResults = [];
function imageCleared(game) { return game.preview?.rightsStatus === 'reviewed_for_catalog_display'; }
function placeholder(game) { const wrap=node('div','missing-preview');wrap.append(node('span','game-glyph',game.genres.includes('racing')?'⌁':'◇'),node('p','',game.preview?'Image rights review pending':'Preview not captured'));return wrap; }
function renderGames() { discovery.change(); }
function drawGames(items, total) {
 currentResults = items;window.HeadStartCollection?.observe(items,'project');
 ...
 for (const game of currentResults) {
  ...
  if(imageCleared(game)){const img=node('img');img.src=game.preview.src;img.alt=game.preview.alt;img.loading='lazy';img.width=640;img.height=400;img.addEventListener('error',()=>{preview.replaceChildren(placeholder(game));},{once:true});preview.append(img,node('span','preview-label','Game image'));}else preview.append(placeholder(game));
  const body=node('div','game-body');body.append(node('h3','',game.title),node('p','game-summary',game.summary));const tags=node('div','game-tags');const sites=(game.platforms||[]).some(p=>p.toLowerCase()==='sites');tags.setAttribute('aria-label','Categories: '+[pretty(game.contentKind),...game.genres.map(pretty),...(sites?['Sites']:[]),platformLabel(game),game.runtime].join(', '));tags.append(node('span','category-chip',pretty(game.contentKind)));if(sites)tags.append(node('span','category-chip','Sites'));for(const genreName of game.genres.slice(0,2))tags.append(node('span','',pretty(genreName)));tags.append(node('span','',platformLabel(game)),node('span','',game.dimension.toUpperCase()));const stars=node('span','github-stars',starLabel(game));stars.title=Number.isInteger(game.githubStars)?game.githubStars.toLocaleString('en')+' stars on the repository · Checked '+(game.popularity?.checked_at||'date unknown').slice(0,10)+' · Shared by subprojects':starLabel(game);stars.setAttribute('aria-label',stars.title);tags.append(stars);const usage=game.notoriety?.metrics?.[0];body.append(tags,node('p','runtime-credit','Runtime: '+game.runtime+' · Interface: '+languageLabel(game)),usage?node('p','runtime-credit',new Intl.NumberFormat('en',{notation:'compact'}).format(usage.value)+' '+pretty(usage.metric_type)+' · publisher-reported'):node('p','runtime-credit','Player count not published'),node('p','model-credit',(game.editorialRank?'Recommended #'+game.editorialRank.position+' · ':'')+modelLabel(game)));
  if(game.buildingBlocks?.length){const systems=node('details','game-systems');const summary=node('summary','',game.buildingBlocks.length+' systems to explore');systems.append(summary,node('p','systems-note','Source leads; scope and integration review required.'));const list=node('ul');for(const block of game.buildingBlocks){const item=node('li');const link=externalLink(block.name+' ↗',block.evidence_url);link.setAttribute('aria-label',block.name+' source evidence (opens in a new tab)');item.append(link);list.append(item);}systems.append(list);body.append(systems);}
  const actions=node('div','game-actions');const demoLink=externalLink(game.label+' ↗',game.demoUrl);demoLink.setAttribute('aria-label',game.label+' for '+game.title+' (opens external website in a new tab)');if(game.label==='Play demo'&&demoLink.tagName==='A'){demoLink.dataset.demoGame=game.id;demoLink.dataset.demoTitle=game.title;}actions.append(demoLink);const add=node('button',selected.has(game.id)?'in-bag':'',selected.has(game.id)?'✓ In bag':'Add to bag');add.type='button';add.dataset.add=game.id;add.setAttribute('aria-label',(selected.has(game.id)?'Remove ':'Add ')+game.title+(selected.has(game.id)?' from bag':' to bag'));add.setAttribute('aria-pressed',String(selected.has(game.id)));add.addEventListener('click',()=>toggleGame(game.id));actions.append(add);const saveProject=window.HeadStartCollection?.button(game,'project');if(saveProject)actions.append(saveProject);body.append(actions);
 ...
 $('result-count').textContent=total+' matching projects · '+currentResults.length+' on this page';$('empty-games').hidden=currentResults.length>0;
}
...
function showSourceBase(game,opener){$('source-title').textContent=game.title;const content=$('source-content');content.replaceChildren();content.append(node('p','',game.creator),externalLink('View pinned source ↗',game.pinnedSourceUrl||game.repoUrl));content.append(node('p','','Category: '+pretty(game.contentKind)+'. Genres: '+game.genres.map(pretty).join(', ')+'.'),node('p','','Runtime: '+game.runtime+' · Interface: '+languageLabel(game)+' · Style: '+game.dimension.toUpperCase()+(game.visualStyle.length?' · '+game.visualStyle.map(pretty).join(', '):'')),node('p','','Reuse review required. Code license: '+game.rights.code_license+'. '+game.rights.asset_notes+' Selected code, assets and dependencies need review for your game.'),node('p','',demoStatus(game)+'. '+game.demoNotes));if(game.editorialRank){content.append(node('h3','','Why recommended'),node('p','',game.editorialRank.rationale),node('p','','Editorial position '+game.editorialRank.position+'. This is a dated discovery order, not an objective quality score.'));}if(game.notoriety?.metrics?.length){content.append(node('h3','','Published usage evidence'));for(const metric of game.notoriety.metrics){const line=node('p','');line.append(externalLink(metric.value.toLocaleString('en')+' '+pretty(metric.metric_type)+' ↗',metric.evidence_url),node('span','',' — '+metric.scope+' Checked '+metric.checked_at.slice(0,10)+'.'));content.append(line);}}if(game.playReview){content.append(node('h3','','Play observation'),node('p','',game.playReview.result),node('p','',game.playReview.scenario),node('p','','Environment: '+game.playReview.environment+'. Limitations: '+game.playReview.limitations));}if(game.buildingBlocks?.length){content.append(node('h3','','Systems to explore'));for(const block of game.buildingBlocks){const line=node('p','');line.append(externalLink(block.name+' ↗',block.evidence_url),node('span','',' — '+block.notes));content.append(line);}} content.append(node('h3','','Popularity signal'),node('p','',game.githubStars.toLocaleString('en')+' GitHub stars · checked '+game.popularity.checked_at.slice(0,10)+'. Count belongs to the repository and is shared by its subprojects.'),externalLink('GitHub count source ↗',game.popularity.evidence_url),node('h3','','Model attribution'),node('p','',modelLabel(game))); if(game.aiProvenance){if(!(game.aiProvenance.evidence||[]).some(e=>e.claim===game.aiProvenance.notes)){content.append(node('p','',game.aiProvenance.notes));}for(const evidence of game.aiProvenance.evidence||[]){const line=node('p','');line.append(externalLink('Creator attribution ↗',evidence.url),node('span','',' — '+evidence.claim));content.append(line);}} content.append(node('p','','Model attribution describes the creator’s reported workflow; it is separate from code rights and successful integration.')); content.append(node('h3','','Game image'),node('p','',game.preview.credit),node('p','','Image review status: '+imageReviewLabel(game.preview)),node('p','','Image rights: '+game.preview.licenseExpression),node('p','',game.preview.allowedUse),node('p','',game.preview.versionRelation),externalLink('Image source ↗',game.preview.sourcePage),externalLink('Rights evidence ↗',game.preview.licenseEvidenceUrl));for(const url of game.preview.licenseUrls){const line=node('p','');line.append(externalLink('Related terms or source ↗',url));content.append(line);}openDialog('source-dialog',opener);}
window.HeadStartDetails.configure({openDialog});
function showSource(game,opener){showSourceBase(game,opener);window.HeadStartDetails.enhance(game);}
const discovery = window.HeadStartDiscovery({ fields: { q:'search-games', genre:'genre', capability:'capability', kind:'content-kind', platform:'platform', style:'style', runtime:'runtime', model:'model', sort:'sort' }, draw:drawGames, onItems:items=>{for(const game of items)gamesById.set(game.id,game);} });
function clearFilters(){for(const id of ['search-games','genre','capability','content-kind','platform','style','runtime','model'])$(id).value='';$('sort').value='recommended';renderGames();}
for(const id of ['genre','capability','content-kind','platform','style','runtime','model','sort'])$(id).addEventListener('change',renderGames);$('search-games').addEventListener('input',discovery.search);$('clear-filters').addEventListener('click',clearFilters);$('reset-games').addEventListener('click',clearFilters);
for(const mode of ['grid','list'])$(mode+'-view').addEventListener('click',()=>discovery.layout(mode));
