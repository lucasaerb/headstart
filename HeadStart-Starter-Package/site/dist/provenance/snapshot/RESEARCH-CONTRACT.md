...
Local display media is keyed to the exact research `record_id`, with the canonical `repo_url` retained as a consistency check. A repository URL alone is not a safe image identity because independently useful monorepo subprojects can share one repository. `rights_status=reviewed_for_catalog_display` records an inspected display basis. `rights_status=official_source_local_display_rights_unresolved` preserves that no broader image license was established and requires separate independent reviewer/timestamp/PASS fields before dashboard projection. `rights_status=candidate_local_display_pending_independent_review` records an authentic media-audit candidate awaiting the required independent review and never projects. Neither unresolved/pending status authorizes publication, promotional use or reuse.
