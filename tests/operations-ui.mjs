...
try{for(const [name,viewport]of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
 ...
 await page.goto(base+'/');const measured=page.waitForResponse(r=>r.url().endsWith('/api/events')&&r.request().method()==='POST');await page.locator('#search-games').fill('camera');await page.locator('#search-games').blur();expect((await measured).status()).toBe(200);await page.goto(base+'/operations.html');await expect(page.locator('#health-status')).toContainText('1 registered');
