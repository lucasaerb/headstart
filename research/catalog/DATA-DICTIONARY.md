...
`media-manifest.json` maps each specific preview to one exact `record_id` and its canonical project reference, original file, file-page revision, declared capture date, download time, original hash/dimensions, credit, rights expression, allowed scope and transformations. Record identity prevents an image from one monorepo subproject from appearing on a different subproject that shares the repository URL. Capture and upstream upload dates can remain unknown; download and author observation times remain timestamped. Historical images do not attest the indexed source commit. Official publication or repository inclusion proves provenance but does not by itself establish image reuse rights, so the manifest can retain broader rights unresolved or independent review pending. Pending candidates stay in the research/contact-sheet review surface and do not project to the dashboard. The plugin review and all source/asset reuse gates remain separate. `media-credits.md` and `media/licenses/` carry notices; evidence snapshots preserve inspected metadata.
