...
def main():
    ...
    parser.add_argument('action',choices=['register-demo','reachability','interactive','register-source','sources','dispatch','retry-source','retry-queue','reviewed-references','status'])
    ...
    try:
        ...
        elif args.action=='reviewed-references':
            from .events import EventStore
            result=EventStore(db,enabled=os.environ.get('HEADSTART_TELEMETRY')=='1').ingest_reviewed_references(Path(__file__).resolve().parents[2],authorized=True)
