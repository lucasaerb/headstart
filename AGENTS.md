...
The next ten-issue implementation runs on `feat/backlog-reuse-batches` in `/tmp/headstart-reuse`, based on stable main `84795b9`. The first five (#12, #27, #13, #14, #15) passed separate author/reviewer checks: `docs/reviews/reuse-batches/auth-handoff-full-review.md` and `batch3-subset-review.md`. Local services now implement submission/correction review, browser-bound verified-email identity with private development delivery preview, rights-checked immutable metadata/notices handoffs, owner-scoped prepared bags, and an actual service-backed stdio MCP connection. Plugin 0.5.0 was exercised in Codex 0.154.0; configure its extracted local copy using the packaged helper, since ambient shell variables alone do not establish client configuration.

These are reviewed localhost capabilities, not production authentication or hosted source execution. The MCP automatically resolves the last explicitly prepared eligible scoped bag; anonymous research collections are not silently synchronized. No source bytes, actual game mashup, royalty agreements or settlements are supplied by these handoffs. C4 assets, main styling and living-world interaction remain the preferred design and are preserved. Batch four (#16–#20) adds bounded integration, reference evidence, review skills, recommendations and health tracking; do not claim those accepted before their independent reviews pass.
