...
- The workspace now includes the supplied specification package, a local C4 frontend, a researched catalog and an installable local discovery plugin. A real read-only bundled MCP connector exists; the read-only catalog API and semantic retrieval are now deployed on Vercel. Verified-email identity, signing and hosted source integration remain unavailable.
...
- `research/catalog/` currently indexes 41 strictly gated browser research/display records and 78 candidate building blocks. Browser/model expansion records distinguish creator attribution from unverified or unknown model provenance, including contribution-specific limits. Collection membership and AI playing a game do not establish model-generated code. Media approval covers narrow catalog identification/display only and does not clear broader asset reuse.
...
The user requested email collection before launching a Play demo on 10 September 2026, then explicitly removed the homepage updates signup. Save the submitted email privately in Vercel before offering the selected external demo; browsing and public upstream links remain open. Demo-access collection does not imply consent to product emails, so keep its separate opt-in unchecked. This does not establish verified identity or satisfy the protected code/handoff gate in issue #27. After durable server confirmation, store a versioned normalized address in that browser, bypass later demo prompts, and provide a visible **Forget email** action. Never create the browser preference after a failed request, expose collected addresses publicly, treat the preference as verified identity, or infer update consent from a demo-only save. No outbound email campaign is authorized by this collection request.
...
The user requested a visible **GitHub** button immediately beside **Get the plugin** in the main header. It links to the canonical public HeadStart repository as a clearly identified external action; it does not replace per-project source links or imply that every catalog item is hosted there.
...
The current backlog branch uses the preserved static C4 frontend with a Node 22 local development server, Python versioned contracts and SQLite metadata/evidence storage. The second batch adds scoped details, a versioned local brief/collection, bounded static intake, 30 curated source-reviewed components and optional local ONNX semantic search. Issues #2–#6 passed independent review at `9a85fde`, and #7–#11 at `7eb0dce`; see `docs/reviews/batches/` for findings, fixes and exact digests. Source-reviewed does not mean integration-tested. See `docs/architecture/development.md` for installation and `docs/planning/batches/README.md` for delivery sequencing. Historical statements above describing an entirely specification-only workspace predate this implementation; a local catalog backend does not establish a deployed catalog, verified identity, MCP integration or royalty service.
...
The current catalog revision contains 41 strictly gated browser research records. All 41 media rows have independent narrow local-display approval; the recent 16-row exact-set decision is recorded in `docs/reviews/recent-vibe-games/media-approval-request.json`.

The user authorized promoting the reviewed catalog release to https://headstart-virid.vercel.app/. Deployment `dpl_8Z4HxboLH8A6YkD9sL7Z6FRsJECM` packages source `3f893341cc69e961f0efb0e9ba806544a4e70978`; the final catalog review is `d431559f9585158f4f60ca7f52b9c73f5a51d414`. The live Python catalog API serves 41 research/display records and 32 eligible scoped versions, with pinned ONNX semantic retrieval. Pre-promotion acceptance is in `docs/reviews/recent-vibe-games/RELEASE-CANDIDATE-REVIEW.md`; post-promotion evidence is in `docs/reviews/recent-vibe-games/PRODUCTION-RELEASE.md`. Source-reviewed is not integration-tested. Demo emails remain in private Vercel Blob, with separate optional marketing consent.
...
## Reviewed reuse branch — 10 September 2026

The ten-issue implementation is on `feat/backlog-reuse-batches` in `/tmp/headstart-reuse`, originally based on `84795b9` and incorporating main's separately approved 41-record catalog refresh through `8d86ad9`. The first five (#12, #27, #13, #14, #15) passed separate author/reviewer checks: `docs/reviews/reuse-batches/auth-handoff-full-review.md` and `batch3-subset-review.md`. Local services now implement submission/correction review, browser-bound verified-email identity with private development delivery preview, rights-checked immutable metadata/notices handoffs, owner-scoped prepared bags, and an actual service-backed stdio MCP connection. The final plugin is 0.7.0 with twelve skills, exercised in Codex 0.154.0; configure its extracted local copy using the packaged helper, since ambient shell variables alone do not establish client configuration.

These are localhost capabilities, not production authentication or hosted source execution. The MCP automatically resolves the last explicitly prepared eligible scoped bag; anonymous research collections are not silently synchronized. No source bytes, actual game mashup, royalty agreements or settlements are supplied by these handoffs. C4 assets, main styling and living-world interaction remain the preferred design and are preserved. Batch four (#16–#20) adds bounded integration, reference evidence, review skills, recommendations and health tracking; consult the exact independent verdicts in `docs/reviews/reuse-batches/` and `docs/planning/reuse-batches/batch4-checklist.md` before claiming acceptance. The ten reference integrations cover declared Three.js scopes; R3F/camera adapters and general cross-engine conversion remain untested. Recipe compositions remain candidates, and reviewed reference counts are not user adoption.

Optional lookup/first-plan instrumentation is a separate checkout-only opt-in documented in `services/operations/INSTRUMENTATION.md`; default packaged execution does not enable it. Consent deletion must stop collection in already-running sessions. Local operational scheduling/storage and development email preview do not establish hosted durable services. The user explicitly authorized wrapping up and merging this work; a merge does not deploy the Vercel site.

Additional checks: `npm run test:auth`, `npm run test:submissions`, `npm run test:handoff`, `npm run test:plugin-setup`, and `npm run test:mcp` (install `tests/mcp/requirements.txt` in a separate test environment and set `HEADSTART_MCP_PYTHON` as needed). Browser checks support `HEADSTART_CHROME_CHANNEL=chrome`. Test credentials, verification previews and raw target material remain private ignored artifacts.

## Research catalog restoration — 11 September 2026

The user explicitly requested all 51 entries removed by the strict browser/source/license/media/stars gates restored beneath the previously visible 41. Current discovery contains 92 research references; the first 41 retain their exact editorial order, and restored references occupy positions 42–92 alphabetically. `records/restored.json` preserves the excluded archive records exactly. Historical exclusion evidence remains unchanged. Discovery metadata may include native games, unresolved source or custom licenses, unavailable demos and unknown stars; this does not promote any source scope, grant reuse permission or weaken verified-email/handoff gates. Existing eligible scoped versions remain 32. Preview images retain their historical evidence; only reviewed-for-catalog-display images render, with explicit placeholders otherwise (52 pictured of 92). See `docs/reviews/catalog-restoration/`.

## Production restoration release — 11 September 2026

Promotion is confirmed for deployment `dpl_85QsyxKo6DDoTsHSs2c4RcSHHF1b`, candidate https://headstart-nqlvskhwm-lucasaerbs-projects.vercel.app, source `636d2ce16748aab2e2205f5f99908112a6ba5b9e`. It packages 92 research references, 52 public images and 32 eligible scoped versions; database SHA256 `49400c6fdec223df4dcab5fba3765eb40cd2f90068fe5616847d2be40cfc54f0`. The implementation is on main at `aef15b3`; the deployable release is isolated from prior production `3f893341cc69e961f0efb0e9ba806544a4e70978` to avoid publishing unrelated localhost-only features. Remote independent checks and promotion evidence are tracked in `docs/reviews/catalog-restoration/PRODUCTION-RELEASE.md`. The production alias https://headstart-virid.vercel.app/ now serves this release. Independent remote desktop/mobile review passed. Post-promotion checks confirmed all 92 records, the unchanged first 41 and restored 51 tail, eight-page pagination with 92 unique IDs, and exact candidate bytes for catalog.js, index.html, app.js and detail.js. Retain the September 10 release evidence unchanged as history.
