...
- Successful homepage and demo saves write the normalized versioned preference; failure and timeout do not.
- Reload prefills and clearly surfaces remembered state; later demo links bypass the prompt without rewriting their URLs.
- Forget clears the preference and restores entry; corrupt/blocked storage is usable and honest.
- Homepage updates always require a deliberate submit, including for remembered demo-only addresses.
...
Intentional deviation: the C4/C4b hero form is removed by current user direction. The art, headline, supporting promise, signup and environment remain; the primary journey becomes a direct browse action.
